...
description: Use quando o aluno precisar do comportamento operacional do agente ECC
  ...
  Use PROACTIVELY after writing code that handles user input, authentication, API
  endpoints, or sensitive data. Flags secrets, SSRF, injection, unsafe crypto, and
...
metadata:
  hermes:
    ...
    - converted-agent
    related_skills: []
    source: https://github.com/affaan-m/ecc/blob/main/agents/security-reviewer.md
---

# Agent convertido: Security Reviewer

## Visão operacional

Esta skill converte o agente ECC `security-reviewer` em um procedimento operacional carregável no Hermes Agent. Use para reproduzir o comportamento do agente dentro da conversa atual, sem depender do mecanismo de agents do ECC/Claude Code.
...
- Agente original: `agents/security-reviewer.md`
