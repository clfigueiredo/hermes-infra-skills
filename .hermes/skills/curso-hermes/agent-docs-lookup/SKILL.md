...
description: Use quando o aluno precisar do comportamento operacional do agente ECC
  `docs-lookup`. When the user asks how to use a library, framework, or API or needs
  up-to-date code examples, use Context7 MCP to fetch current documentation and return
...
metadata:
  hermes:
    ...
    - converted-agent
    related_skills: []
    source: https://github.com/affaan-m/ecc/blob/main/agents/docs-lookup.md
---

# Agent convertido: Docs Lookup

## Visão operacional

Esta skill converte o agente ECC `docs-lookup` em um procedimento operacional carregável no Hermes Agent. Use para reproduzir o comportamento do agente dentro da conversa atual, sem depender do mecanismo de agents do ECC/Claude Code.
...
- Agente original: `agents/docs-lookup.md`
